...
#
#   1. Scrapea y agrupa productos con Playwright  -> /tmp/productos_ar.json
...
OUT=/tmp/productos_ar.json
echo "· Buscando productos ganadores (Argentina)…"
...
cp "$OUT" /tmp/ec-datos/productos_ar.json
( cd /tmp/ec-datos
  git add productos_ar.json
